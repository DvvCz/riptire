...
function Appearance() {
	const [checked, setChecked] = createStoredSignal("..test", true);
	const [navColor, setNavColor] = createStoredSignal("v1.theme.nav", "#FFFFFF");
	const [primColor, setPrimColor] = createStoredSignal("v1.theme.primary", "#FFFFFF");
	const [secColor, setSecColor] = createStoredSignal("v1.theme.secondary", "#000000");
	const [terColor, setTerColor] = createStoredSignal("v1.theme.tertiary", "#737373");

	return (
		<div class="flex flex-row gap-4">
			<div class="flex flex-col items-center rounded-lg bg-black/10 w-1/3 p-4 gap-4">
				<span class="text-lg font-semibold mb-6">Theme</span>
				...
				<div class="flex flex-row justify-between w-full items-center">
					<span id="color-sec">Secondary</span>
					<input
				...
				<div class="flex flex-row justify-between w-full items-center">
					<span id="color-tert">Tertiary</span>
					<input
			...
			<div class="flex flex-col items-center rounded-lg bg-black/10 w-1/3 p-4 gap-4">
				<span class="text-lg font-semibold mb-6">Switches</span>

				<Switch checked={checked()} onChange={setChecked} class="flex flex-row justify-between w-full items-center">
		...
		<div class="flex p-8 justify-center items-center">
			<div class="flex flex-col gap-10">
				<div class="flex flex-col gap-4">
					...
					<Show when={!subs.loading} fallback={<div> Loading... </div>}>
						{(() => {
							const data = subs()!;

							const ids = Object.keys(data);
							if (ids.length === 0) {
								return <div class="text-center">Nothing here.</div>;
							}

							return (
								<div class="flex flex-col w-full gap-8 items-center">
									<textarea
										class="rounded-lg resize-none border-black drop-shadow-md outline-hidden p-4"
										value={ids.join("\n")}
										rows={10}
										cols={60}
										readonly
...
export default function Settings() {
	const [section, setSection] = createSignal<(typeof SECTIONS)[number]>("Appearance");
